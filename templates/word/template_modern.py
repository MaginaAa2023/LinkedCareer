...
set_font(run, size=9, color=(230,230,230))
...
p = doc.add_paragraph()
run = p.add_run("◆ 核心优势")
set_font(run, size=12, bold=True, color=(41, 128, 185))
p = doc.add_paragraph("—" * 70)
set_font(p.runs[0], size=9, color=(74, 144, 226))
...
for border in ['top', 'left', 'bottom', 'right']:
    borderElm = OxmlElement(f'w:{border}')
    borderElm.set(qn('w:val'), 'single')
    borderElm.set(qn('w:sz'), '4')
    borderElm.set(qn('w:color'), 'E4E7ED')
    tcBorders.append(borderElm)
tcPr.append(tcBorders)
...
set_font(run, size=11, bold=True, color=(41, 128, 185))
run = p.add_run("                          2026.01 - 至今")
set_font(run, size=10, color=(96, 96, 96))
...
p = doc.add_paragraph("• 2020年入选龙湖合伙人，2024-2025获评高级合伙人，集团2万员工前80\n")
set_font(run, size=10.5)
...
set_font(run, size=10.5)
run = p.add_run(" 10亿元")
set_font(run, size=10.5, bold=True, color=(217, 66, 66))
...
p = doc.add_paragraph("• 内容营销整合破冰项目（2019）：日均收入从25万提升至")
set_font(run, size=10.5)
run = p.add_run(" 150万")
set_font(run, size=10.5, bold=True, color=(217, 66, 66))
...
p = doc.add_paragraph()
run = p.add_run("• 产品能力：")
set_font(run, size=10.5, bold=True, color=(41, 128, 185))
...
# 保存
output_path = "郝霄_互联网现代风格简历.docx"
doc.save(output_path)
print(f"✅ 现代风格简历生成成功：{output_path}")
