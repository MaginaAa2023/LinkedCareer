...
`
  ...
  async generateProfessionalPDF(careerData, outputPath, options = {}) {
    ...
    // 读取HTML模板
    let template = await fs.readFile(path.join(__dirname, '../../templates/resume_professional.html'), 'utf8')
    
    // 预处理数据：量化业绩自动加粗
    const processedExperiences = careerData.experiences.map(exp => {
      const processedAchievements = exp.achievements.map(ach => {
        // 数字自动加粗
        return ach.replace(/(\d+(\.\d+)?[%亿万元千])/g, '<span class="highlight">$1</span>')
      })
      return { ...exp, achievements: processedAchievements }
    })

    // 提取核心亮点
    const highlights = this.extractCoreHighlights(careerData)

    // 简单模板渲染
    template = template.replace(/{{name}}/g, careerData.basicInfo.name)
    template = template.replace(/{{phone}}/g, careerData.basicInfo.phone)
    template = template.replace(/{{email}}/g, careerData.basicInfo.email)
    template = template.replace(/{{city}}/g, careerData.basicInfo.city)
    template = template.replace(/{{expectedPosition}}/g, careerData.basicInfo.expectedPosition || '')
    
    // 渲染核心亮点
    if (highlights.length > 0) {
      let highlightsHtml = '<ul class="list-disc pl-5">'
      highlights.forEach(h => {
        highlightsHtml += `<li>${h}</li>`
      })
      highlightsHtml += '</ul>'
      template = template.replace(/{{#if highlights}}[\s\S]*?{{\/if}}/, `
      <div class="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
        <div class="font-bold text-blue-800 mb-1">核心亮点</div>
        ${highlightsHtml}
      </div>
      `)
    } else {
    ...
    processedExperiences.forEach(exp => {
      ...
      <div class="mb-4">
        <div class="flex justify-between">
          ...
          <div class="text-gray-600">${exp.startTime} - ${exp.endTime}</div>
        </div>
        <div class="mb-1"><strong>工作职责：</strong>${exp.description}</div>
        <div><strong>主要业绩：</strong></div>
        <ul class="list-disc pl-5">
          ${exp.achievements.map(ach => `<li class="achievement-item">${ach}</li>`).join('')}
        </ul>
      </div>
      `
    })
    template = template.replace(/{{#each experiences}}[\s\S]*?{{\/each}}/, expHtml)

    // 渲染核心技能
    let skillHtml = ''
    for (const [category, skills] of Object.entries(careerData.skills || {})) {
      skillHtml += `
      <div class="mb-2">
    ...
    template = template.replace(/{{#each skills}}[\s\S]*?{{\/each}}/, skillHtml)
    ...
    careerData.education.forEach(edu => {
      ...
      <div class="mb-2">
        <div class="flex justify-between">
          <strong>${edu.school} · ${edu.major} · ${edu.degree || '本科'}</strong>
          <div class="text-gray-600">${edu.startTime} - ${edu.endTime}</div>
        </div>
      </div>
      `
    })
    template = template.replace(/{{#each education}}[\s\S]*?{{\/each}}/, eduHtml)
  ...
  async generateProfessionalWord(careerData, outputPath, options = { industry: 'internet' }) {
    ...
    // 生成带样式的Markdown
    let mdContent = `# ${careerData.basicInfo.name}
### 联系方式
电话：${careerData.basicInfo.phone} | 邮箱：${careerData.basicInfo.email} | 所在城市：${careerData.basicInfo.city} | 期望岗位：${careerData.basicInfo.expectedPosition || ''}

---
## 核心亮点
${this.extractCoreHighlights(careerData).map(h => `- ${h}`).join('\n')}
...
**时间：** ${exp.startTime} - ${exp.endTime}
**工作职责：** ${exp.description}
**主要业绩：**
...
`
    for (const [category, skills] of Object.entries(careerData.skills || {})) {
      mdContent += `- **${category}**: ${skills.join('、')}\n`
    }
...
`
    careerData.education.forEach(edu => {
      mdContent += `- **${edu.school}** · ${edu.major} · ${edu.degree || '本科'}（${edu.startTime} - ${edu.endTime}）\n`
    })

    // 保存临时Markdown文件
    const tempMdPath = path.join('/tmp', `resume_${Date.now()}.md`)
    await fs.writeFile(tempMdPath, mdContent, 'utf8')

    // 选择对应行业的Word模板
    const templateMap = {
      'internet': path.join(__dirname, '../../templates/word_template_internet.dotx'),
      'finance': path.join(__dirname, '../../templates/word_template_finance.dotx'),
      'soe': path.join(__dirname, '../../templates/word_template_soe.dotx'),
      'executive': path.join(__dirname, '../../templates/word_template_executive.dotx')
    }
    const templatePath = templateMap[options.industry] || templateMap.executive

    // 调用pandoc生成Word
    execSync(`pandoc -s "${tempMdPath}" -o "${outputPath}" --reference-doc "${templatePath}"`, { stdio: 'ignore' })
